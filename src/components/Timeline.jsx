...
export default function Timeline() {
    ...
    return (
        <section id="timeline" style={{ padding: "6rem 2rem", position: "relative", zIndex: 1, width: "100%", margin: "0 auto" }}>
            <SectionHeader title="Parcours" subtitle="Expériences professionnelles & formations" accent={C.violet} />

            {/* Filter tabs */}
            <div style={{ display: "flex", gap: "0.6rem", justifyContent: "center", marginBottom: "3rem", flexWrap: "wrap" }}>
                {[{ v: "all", l: "✨ Tout", c: C.violet }, { v: "work", l: "💼 Expériences", c: C.blue }, { v: "edu", l: "🎓 Formations", c: C.cyan }].map(f => (
            ...
            <div style={{ position: "relative", width: "100%" }}>
                {/* vertical line - rainbow gradient */}
                <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 2, background: `linear-gradient(to bottom, ${C.blue}, ${C.violet}, ${C.pink}, ${C.cyan})`, transform: "translateX(-50%)", borderRadius: 2, opacity: 0.5 }} />
                {data.map((item, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: i % 2 === 0 ? "flex-start" : "flex-end", marginBottom: "2.5rem", position: "relative", width: "100%" }}>
                        {/* dot */}
                        <div style={{ position: "absolute", left: "50%", top: "1.4rem", transform: "translateX(-50%)", width: 16, height: 16, borderRadius: "50%", background: item.color, border: `3px solid ${C.bg}`, boxShadow: `0 0 16px ${item.color}88`, zIndex: 2 }} />
                        <div style={{ width: "45%", background: C.bgCard, border: `1px solid ${item.color}30`, borderRadius: 14, padding: "1.3rem 1.5rem", backdropFilter: "blur(10px)", transition: "all 0.3s", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}
                            onMouseEnter={e => { e.currentTarget.style.borderColor = item.color; e.currentTarget.style.transform = "scale(1.02)"; e.currentTarget.style.boxShadow = `0 10px 30px ${item.color}30`; }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor = item.color + "30"; e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,0.06)"; }}
                        >
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
                                <span style={{ fontSize: "1.3rem" }}>{item.icon}</span>
                                <span style={{ fontSize: "0.7rem", color: item.color, fontFamily: "'Lato', sans-serif", background: item.color + "18", padding: "3px 10px", borderRadius: 10, fontWeight: 600 }}>{item.period}</span>
                            </div>
